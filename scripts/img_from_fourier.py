...
def _build_parser():
    ...
    # TODO add late interpolation feature
    ...
    help="""Amount of individual points along the curve to render. Higher values increase quality (rapidly) 
...
if __name__ == "__main__":
    ...
    with open(args.fourier) as ff: # TODO use CSV format
    ...
    X_pix = np.clip(np.round(XY[:,0]),0,res_x-1).astype(dtype=int)
    Y_pix = np.clip(np.round(XY[:,1]),0,res_y-1).astype(dtype=int)
